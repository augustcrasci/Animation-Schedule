...
from app.metadata_repair import next_repair_year, repair_metadata_year
...
ACTION_META = {
    "viewer": {
        "title": "현재 DB로 뷰어 열기",
        "detail": "저장된 DB만 사용해서 바로 뷰어를 엽니다.",
        "button": "뷰어 열기",
    },
    "update_fast": {
        "title": "빠른 최신 갱신",
    ...
    "update": {
        "title": "최신 시즌 풀 재동기화",
        "detail": "이번 분기와 미래 분기를 링크/이름 보강까지 포함해 다시 갱신합니다.",
        "button": "풀 재동기화",
    },
    "update_and_viewer": {
        "title": "풀 재동기화 후 뷰어 열기",
        "detail": "최신 시즌 풀 재동기화가 끝나면 바로 뷰어를 엽니다.",
        "button": "갱신 후 열기",
    },
    "backfill_year": {
        "title": "과거 1년치 추가 수집",
        "detail": "과거 시즌 4개를 더 쌓습니다.",
        "button": "과거 1년 추가",
    },
    "repair_metadata_year": {
        "title": "과거 1년치 메타데이터 복구",
        "detail": "연도 1개 분량의 공식 링크와 캐스트/스태프 빈칸을 보수합니다.",
        "button": "1년 복구",
    },
}
...
def format_year_range(years: list[int] | None) -> str:
    values = sorted(int(item) for item in (years or []) if isinstance(item, int) or str(item).isdigit())
    if not values:
        return "湲곕줉 ?놁쓬"
    if len(values) == 1:
        return str(values[0])
...
def load_launcher_status() -> dict[str, str]:
    ...
    return {
        ...
        "next_repair_year": str(next_repair_year() or "기록 없음"),
    }
...
class LauncherApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.detail_var = tk.StringVar(value="최신 갱신, 전체 재동기화, 과거 백필, 과거 메타데이터 복구를 분리해서 실행할 수 있습니다.")
        ...
        for mode in ("viewer", "update_fast", "update", "update_and_viewer", "backfill_year", "repair_metadata_year"):
    ...
    def refresh_status_panel(self) -> None:
        ...
        if has_key:
            self.credential_var.set("본인 MAL Client ID가 설정되어 있습니다. 아래 입력칸에서 다른 ID로 바꿔 저장할 수도 있습니다.")
        else:
            self.credential_var.set("MAL Client ID가 없습니다. 아래 입력칸에 붙여넣고 저장하면 DB 업데이트가 활성화됩니다.")

        self.db_status_var.set(
            f"최신 시즌 갱신 기준: {status['last_live_sync']} | 현재 저장 엔트리 수(영화 포함): {status['total_entries']}"
        )
        self.backfill_var.set(
            f"과거 백필 완료 범위: {status['oldest_completed']}까지 | 다음 백필 목표: {status['next_backfill']} | 다음 복구 연도: {status['next_repair_year']}"
        )
    ...
    def handle_save_client_id(self) -> None:
        ...
        self.refresh_status_panel()
        self._apply_progress("MAL Client ID를 저장했습니다.", "이제 최신 갱신과 과거 백필, 메타데이터 복구 버튼을 사용할 수 있습니다.")
    ...
    def run_mode(self, mode: str) -> None:
        try:
            ...
            if mode == "repair_metadata_year":
                repaired = repair_metadata_year(progress_callback=self.set_progress)
                if repaired is None:
                    self.root.after(0, lambda: self.finish_update("복구할 과거 연도가 없습니다."))
                else:
                    self.root.after(0, lambda: self.finish_update(f"{repaired}년 메타데이터 복구가 완료되었습니다."))
                return
    ...
    def finish_update(self, message: str) -> None:
        self.set_busy(False)
        self._apply_progress(message, "현재 DB 상태와 복구 범위를 다시 읽어왔습니다.")
    ...
    def show_error(self, exc: Exception) -> None:
        self.set_busy(False)
        message = str(exc).strip() or "예상하지 못한 오류가 발생했습니다. 다시 시도해 주세요."
...
def load_launcher_status() -> dict[str, str]:
    ...
    total_entries = summary.get("total_entries")
    total_entries_text = f"{total_entries:,}개 엔트리" if isinstance(total_entries, int) else "기록 없음"
    repaired_years = collector_status.get("repaired_metadata_years") or source_state.get("repaired_metadata_years") or []
    last_repair_year = collector_status.get("last_metadata_repair_year") or source_state.get("last_metadata_repair_year")
    last_repair_at = collector_status.get("last_metadata_repair_at") or source_state.get("last_metadata_repair_at")
    ...
    return {
        ...
        "total_entries": total_entries_text,
        "next_repair_year": str(next_repair_year() or "기록 없음"),
        "repair_range": format_year_range(repaired_years),
        "last_repair_year": str(last_repair_year or "기록 없음"),
        "last_repair_at": format_timestamp(last_repair_at),
    }


def _refresh_status_panel_v2(self: LauncherApp) -> None:
    status = load_launcher_status()
    has_key = has_mal_client_id()
    ...
    self.build_var.set(
        f"메타데이터 복구 완료 범위: {status['repair_range']} | 마지막 복구 연도: {status['last_repair_year']} | 다음 복구 연도: {status['next_repair_year']}\n"
        f"마지막 복구 시각: {status['last_repair_at']} | 마지막 compiled 빌드: {status['last_build']}"
    )
